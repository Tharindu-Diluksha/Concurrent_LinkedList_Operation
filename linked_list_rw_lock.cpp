...
void Test::read_init_file(int case_no,int m)
{
	...
	std::fstream myfile("rw_lock_init_"+to_string(case_no), std::ios_base::in);
	if (myfile.is_open()){
		int line_no = 0;
		int number;
		while ( myfile >> number)
...
void Test::read_case_file(int case_no, int m)
{
	...
	ifstream myfile ("rw_lock_case_"+to_string(case_no));
	if (myfile.is_open())
	{
		//int line_no=0;
...
void Test::generate_and_populate_op_list(int case_no,int n,int m,float member_p,float insert_p,float delete_p)
{
    ...
    case_commands.open("rw_lock_case_"+to_string(case_no));
    init_population.open("rw_lock_init_"+to_string(case_no));
